...
class GymWrapper(py_environment.PyEnvironment):
  ...
  def _step(self, action):
    ...
    # GK: in case the action is discrete(n) (i.e. scalar) then action is array(<value>)
    # i.e. a scalar numpy . this data type is not hashable and cant be used as index or key to python data
    # structrues. in FrozenLake it is used as key to transition table so returns an error.
    # thus, here we check if the action space is discrete and scalar and if so we pass np.asscalar(action)
    # action = np.asscalar(action) if self._action_is_discrete and np.ndim(self.action_spec())==0 else action
    action = np.asscalar(action) if self._action_is_discrete else action
